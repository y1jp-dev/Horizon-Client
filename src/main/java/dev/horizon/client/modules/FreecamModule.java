...
public class FreecamModule extends Module {
    ...
    public final Setting<Boolean> cameraMine = addSetting(
            new Setting<>("CameraMine", "Mine/interact at the freecam camera position", false));
    ...
    public boolean playerWasForward, playerWasBackward;
    public boolean playerWasLeft,    playerWasRight;
    public boolean playerWasJumping, playerWasSneaking;
    public boolean playerWasSprinting;
    ...
    protected void onEnable() {
        ...
        playerWasForward   = mc.options.forwardKey.isPressed();
        playerWasBackward  = mc.options.backKey.isPressed();
        playerWasLeft      = mc.options.leftKey.isPressed();
        playerWasRight     = mc.options.rightKey.isPressed();
        playerWasJumping   = mc.options.jumpKey.isPressed();
        playerWasSneaking  = mc.options.sneakKey.isPressed();
        playerWasSprinting = mc.player.isSprinting();

        movingForward  = playerWasForward;
        movingBackward = playerWasBackward;
        movingLeft     = playerWasLeft;
        movingRight    = playerWasRight;
        movingUp       = playerWasJumping;
        movingDown     = playerWasSneaking;
    }
    ...
    protected void onDisable() {
        ...
        if (mc.options != null) {
            ...
            KeyBinding.setKeyPressed(InputUtil.fromTranslationKey(mc.options.forwardKey.getBoundKeyTranslationKey()), false);
            KeyBinding.setKeyPressed(InputUtil.fromTranslationKey(mc.options.backKey.getBoundKeyTranslationKey()), false);
            KeyBinding.setKeyPressed(InputUtil.fromTranslationKey(mc.options.leftKey.getBoundKeyTranslationKey()), false);
            KeyBinding.setKeyPressed(InputUtil.fromTranslationKey(mc.options.rightKey.getBoundKeyTranslationKey()), false);
            KeyBinding.setKeyPressed(InputUtil.fromTranslationKey(mc.options.jumpKey.getBoundKeyTranslationKey()), false);
            KeyBinding.setKeyPressed(InputUtil.fromTranslationKey(mc.options.sneakKey.getBoundKeyTranslationKey()), false);
        }
        resetCameraMovement();
        resetPlayerSnapshot();
    }
    ...
    private void resetPlayerSnapshot() {
        playerWasForward = playerWasBackward = false;
        playerWasLeft    = playerWasRight    = false;
        playerWasJumping = playerWasSneaking = playerWasSprinting = false;
    }
}
