    ...
    public void render(WorldRenderContext context) {
        ...
        if (fa > 0f) {
            ...
            for (BlockPos p : positions) {
                ...
                fillBox(buf, mat, x1, y1, z1, x1+1, y1+1, z1+1, FR, FG, FB, fa);
            }
        ...
        if (outline.getValue()) {
            ...
            for (BlockPos p : positions) {
                double x1 = p.getX(), y1 = p.getY(), z1 = p.getZ();
                outlineBox(buf, mat, x1, y1, z1, x1+1, y1+1, z1+1, OR, OG, OB, 1f);
            }
        ...
        if (tracers.getValue()) {
            ...
            BufferBuilder buf = tess.begin(VertexFormat.DrawMode.DEBUG_LINES, VertexFormats.POSITION_COLOR);
            for (BlockPos p : positions) {
                v(buf, mat, ox, oy, oz, OR, OG, OB, 0.7f);
                v(buf, mat, p.getX()+.5, p.getY()+.5, p.getZ()+.5, OR, OG, OB, 0.7f);
            }
