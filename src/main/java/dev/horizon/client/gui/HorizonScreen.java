...
/**
 * Horizon Client GUI Screen.
 * Lightweight pure-fill rendering — works on OpenGL and Vulkan (Sodium/Iris).
 */
public class HorizonScreen extends Screen {
    ...
    private static final int C_BIND_WAITING = 0xFF3A3A1A; // yellowish tint when listening
    ...
    private Module bindingModule      = null; // non-null = waiting for key press

    // Slider drag
    private Setting<?> draggingSetting = null;
    private int sliderX, sliderW;

    // Per-frame slider Y tracking (rebuilt each render)
    ...
    // Per-frame keybind row Y (rebuilt each render)
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        // "Listening…" overlay if binding
        if (bindingModule != null) {
            drawBindOverlay(ctx);
        }

        // Do NOT call super.render() - triggers Minecraft gaussian blur in 1.21.4
    ...
    // ─────────────────────────────────────────────────────────────
    //  KEY EVENTS — keybind capture
    // ─────────────────────────────────────────────────────────────
    @Override
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (bindingModule != null) {
            if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
                // Cancel — leave existing bind untouched
                bindingModule = null;
        ...
        // ESC closes the screen normally
        if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
            this.close();
            return true;
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (int i = 0; i < modules.size(); i++) {
            ...
            if (isHov(mx, my, sx, rowY, mw, MODULE_H)) {
                if (button == 0) selectedModule = mod;
        ...
        if (selectedModule != null) {
            ...
            // Increments MUST match drawSetting() return values + the +6 gap the render loop adds.
            // BOOLEAN: drawSetting returns y+14, caller adds +6 => total advance = 20
            // SLIDER:  drawSetting returns y+24, caller adds +6 => total advance = 30
            int settingY = ry + 32;
            for (Setting<?> s : selectedModule.getSettings()) {
                if (s.getType() == Setting.SettingType.BOOLEAN) {
                    ...
                    settingY += 20; // 14 (drawSetting) + 6 (gap)
                    ...
                    if (isHov(mx, my, rx, ty - 4, tw, 12)) {
                        ...
                        ((Setting<Double>) s).setValue(min + pct * (max - min));
                        draggingSetting = s;
                        sliderX = rx; sliderW = tw;
                        return true;
                    }
                    settingY += 30; // 24 (drawSetting) + 6 (gap)
                }
                // NOTE: do NOT add +6 here again — it is already included above
            }

            // Keybind button (dynamic Y from last render)
            if (keybindRowY >= 0) {
                String bindLabel = "[" + selectedModule.getKeybindName() + "]";
                int bindW = textRenderer.getWidth(bindLabel);
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dX, double dY) {
        if (draggingSetting != null && draggingSetting.getType() == Setting.SettingType.SLIDER) {
            ...
            ((Setting<Double>) draggingSetting).setValue(min + pct * (max - min));
            return true;
        }
    ...
    private static void fillCorner(DrawContext ctx, int ox, int oy, int r, int color,
        ...
        for (int row = 0; row < r; row++) {
            ...
            // Use the actual pixel distance from the circle centre to compute the chord half-width.
            // Previously (r - row - 1) was used, which inverted the arc direction.
